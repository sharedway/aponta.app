...
class NotificationTokenModelCollectionConsumer(AsyncJsonWebsocketConsumer):
    model_meta = NotificationTokenModel._meta
    

    @property
    def nome_do_grupo(self):
        return f"notifications_collection_notificationtokenmodel"
...
class NotificationTokenModelConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    model_meta = NotificationTokenModel._meta
    
    @property
    def nome_do_grupo(self):
        return f"NotificationTokenModel_{self.object_pk}" 
    ...
    async def connect(self):
        ...
        if self.user.is_authenticated:
            ...
            if can_view:                
                ...
                await self.channel_layer.group_add(self.nome_do_grupo, self.channel_name)        
                await self.accept()
                app.send_task("stream_live_update_notificationtokenmodel",[self.object_pk])
        else:
            await self.close()
