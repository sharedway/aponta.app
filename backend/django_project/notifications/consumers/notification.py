...
class NotificationModelCollectionConsumer(AsyncJsonWebsocketConsumer):
    model_meta = NotificationModel._meta
    

    @property
    def nome_do_grupo(self):
        return f"notifications_collection_notificationmodel"
...
class NotificationModelConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    model_meta = NotificationModel._meta
    
    @property
    def nome_do_grupo(self):
        return f"NotificationModel_{self.object_pk}" 
    ...
    async def connect(self):
        ...
        if self.user.is_authenticated:
            ...
            if can_view:                
                ...
                await self.channel_layer.group_add(self.nome_do_grupo, self.channel_name)        
                await self.accept()
                app.send_task("stream_live_update_notificationmodel",[self.object_pk])
        else:
            await self.close()
