...
class AppImagemCollectionConsumer(AsyncJsonWebsocketConsumer):
    model_meta = AppImagem._meta
    

    @property
    def nome_do_grupo(self):
        return f"aplicativos_collection_appimagem"
...
class AppImagemConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    model_meta = AppImagem._meta
    
    @property
    def nome_do_grupo(self):
        return f"AppImagem_{self.object_pk}" 
    ...
    async def connect(self):
        ...
        if self.user.is_authenticated:
            ...
            if can_view:                
                ...
                await self.channel_layer.group_add(self.nome_do_grupo, self.channel_name)        
                await self.accept()
                app.send_task("stream_live_update_appimagem",[self.object_pk])
        else:
            await self.close()
