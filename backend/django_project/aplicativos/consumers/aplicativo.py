...
class AplicativoModelCollectionConsumer(AsyncJsonWebsocketConsumer):
    model_meta = AplicativoModel._meta
    

    @property
    def nome_do_grupo(self):
        return f"aplicativos_collection_aplicativomodel"
...
class AplicativoModelConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    model_meta = AplicativoModel._meta
    
    @property
    def nome_do_grupo(self):
        return f"AplicativoModel_{self.object_pk}" 
    ...
    async def connect(self):
        ...
        if self.user.is_authenticated:
            ...
            if can_view:                
                ...
                await self.channel_layer.group_add(self.nome_do_grupo, self.channel_name)        
                await self.accept()
                app.send_task("stream_live_update_aplicativomodel",[self.object_pk])
        else:
            await self.close()
