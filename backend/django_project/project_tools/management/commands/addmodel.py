...
from ._private import APP_URLS_TEMPLATE,APP_URLS_INIT_TEMPLATE,ADMIN_INIT_TEMPLATE, ADMIN_TEMPLATE,CHANNELS_TEMPLATE, CHANNELS_INIT_TEMPLATE, MODEL_TEMPLATE, TASK_TEMPLATE, VIEW_TEMPLATE, SIGNAL_TEMPLATE, SERIALIZER_TEMPLATE, MODEL_INIT_TEMPLATE, SIGNAL_INIT_TEMPLATE, VIEW_INIT_TEMPLATE, TASK_INIT_TEMPLATE, SERIALIZER_INIT_TEMPLATE, WEB_URLS_TEMPLATE, WEB_URLS_INIT_TEMPLATE, ASYNC_URLS_INIT_TEMPLATE, ASYNC_URLS_TEMPLATE
    ...
    def handle(self, *args, **options):
        ...
        self.render_context.update(
            {
                ...
                "STREAM_OBJECT_TASK_NAME":f"stream_live_update_{self.model_name.lower()}",
                "STREAM_COLLECTION_TASK_NAME":f"{self.app_name}_collection_{self.model_name.lower()}",
                "model_name":self.model_name,
                "verbose_name_plural":self.verbose_name,
        ...
        if os.path.exists(f"{self.base_dir}"):
            for modelo in modelos:
                self.create_file(base_dir=f"{self.base_dir}/{modelo['modulo']}",save_to=self.save_to,template_in=modelo['template_in'], init_in=modelo['init_in'])
